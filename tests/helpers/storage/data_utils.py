...
def create_sample_dataframe(
    start_time: datetime, periods: int, freq: str = "T"
) -> pd.DataFrame:
    ...
    timestamps = pd.date_range(
        start=start_time, periods=periods, freq="min", tz=timezone.utc
    )
...
def generate_test_data(
    start_time: datetime, end_time: datetime, freq_minutes: int = 60
) -> pd.DataFrame:
...
def find_time_gaps(
    df: pd.DataFrame, expected_freq=None, timestamp_col: str = "timestamp"
):
    ...
    expected = pd.date_range(
        start=ts.iloc[0], end=ts.iloc[-1], freq=expected_freq, tz="UTC"
    )
    ...
    if not missing.empty:
        ...
        for k, g in groupby(enumerate(missing_list), lambda ix: ix[0] - ix[1].value):
            ...
            gaps.append(
                (
                    pd.Timestamp(group[0]).tz_convert("UTC"),
                    pd.Timestamp(group[-1]).tz_convert("UTC"),
                )
            )
    return gaps
