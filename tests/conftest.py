...
def log_relevant_env_vars(logger_instance, context_message=""):
    ...
    relevant_vars = {k: v for k, v in os.environ.items() if k.startswith(("STORAGE_", "CCXT_", "PYTEST_"))}
    if not relevant_vars:
        logger_instance.info("No relevant (STORAGE_*, CCXT_*, PYTEST_*) environment variables found.")
    else:
...
async def azure_backend() -> AsyncGenerator[IStorageBackend, None]: # Async fixture
    ...
    backend = AzureBlobBackend(container_name=unique_container_name, use_managed_identity=use_identity, account_name=az_account_name)

    # Initialize the container client to ensure service client is available
    await backend._get_container_client()
    container_mgmt_client = backend._service_client
    ...
    try:
        ...
        if backend:
            try:
                print(f"Attempting to close AzureBlobBackend instance for container: {unique_container_name}")
                await backend.close() 
                print(f"Closed AzureBlobBackend instance for container: {unique_container_name}")
            except Exception as e:
                print(f"Warning: Error closing AzureBlobBackend instance for {unique_container_name}: {e}")

        try:
            print(f"Attempting to delete test container: {unique_container_name}")
            await asyncio.sleep(2) 
            await container_mgmt_client.delete_container(unique_container_name)
        ...
        try:
            print(f"Attempting to close container management BlobServiceClient for: {unique_container_name}")
            await container_mgmt_client.close()
            print(f"Closed container management BlobServiceClient for: {unique_container_name}")
        except Exception as e:
            print(f"Warning: Error closing container management BlobServiceClient for {unique_container_name}: {e}")

        # Add cleanup of test folders in the original container as well
        try:
            await asyncio.sleep(1)  # Give Azure time to process
            await _cleanup_azure_test_data_async(base_container_name, az_account_name)
        except Exception as e:
            print(f"Warning: Error cleaning up test data in {base_container_name}: {e}")
        ...
        async with BlobServiceClient(account_url=account_url, credential=credential) as blob_service_client:
            ...
            if test_folders:
                ...
                for folder in test_folders:
                    try:
                        deleted_count = 0
                        async for blob in container_client.list_blobs(name_starts_with=f"{folder}/"):
                            try:
                        ...
                        if deleted_count > 0:
                            print(f"Cleaned up folder '{folder}' ({deleted_count} items)")
                    except Exception as e:
...
@pytest.fixture(scope="function")
async def storage_manager(request, local_backend, azure_backend, test_path_strategy) -> AsyncGenerator[IStorageManager[IExchangeRecord], None]:
    ...
    manager = OHLCVStorageManager(backend=current_backend, **make_strategy_kwargs(current_backend))
    yield manager
    # No explicit close for manager here; its backend is closed by its own fixture (local_backend/azure_backend)

@pytest.fixture(scope="function")
async def setup_historical_data(request, local_backend, azure_backend, test_path_strategy) -> AsyncGenerator[dict, None]:
    """
    ...
    log_relevant_env_vars(logger, context_message="Inside setup_historical_data before settings load")
    # Get backend type from request parameter (passed via indirect parametrization)
    backend_type = getattr(request, 'param', 'local')  # Default to local if no param
    ...
    if backend_type == "local":
        ...
        raise ValueError(f"Unknown backend type requested for setup_historical_data: {backend_type}")
    ...
    setup_sm = OHLCVStorageManager(backend=current_backend_for_setup, **make_strategy_kwargs_for_setup(current_backend_for_setup))
    ...
    print(f"Setup ({backend_type}): Fetching data for {context['coin']} from {start_time} to {end_time}")
    try:
        ...
        if exchange_data.data:
            ...
            print(f"Setup ({backend_type}): Saved {len(exchange_data.data)} records to storage.")
            yield context
        else:
...
def check_no_data_dirs_after_tests():
    ...
    if found:
        print(f"[WARNING] The following data directories exist after tests: {found}")
        ...
        try:
            ...
            for folder in folders:
                try:
                    ...
                    print(f"Successfully deleted folder '{folder}' ({deleted_count} items)")
                except Exception as e:
    ...
    def _cleanup_azure_test_folders():
        try:
            ...
            if not azure_account_name or not azure_container_name:
                print("Missing Azure account name or container name, skipping Azure cleanup")
                return
