...
async def test_integration_check_coin_availability_fail(client):
    ...
    result = await client.check_coin_availability(
        "INVALID/SYMBOL"
    )  # Await the coroutine
...
async def test_fetch_ohlcv_data_basic(client):
    ...
    interval = "5m"
    result = await client.fetch_ohlcv_data(
        "BTC/USD", start_time, end_time, interval=interval
    )
...
async def test_integration_fetch_two_weeks_five_minute_data(client):
    ...
    logger.info(
        f"Fetching two weeks of 5m data for BTC/USD from {start_dt} to {end_dt}"
    )
    result = await client.fetch_ohlcv_data(
        "BTC/USD", start_dt, end_dt, interval=interval
    )
    ...
    expected_min_candles = (
        2 * 7 * 24 * 12 * 0.9
    )  # Allow for some downtime/missing data (10% buffer)
    expected_max_candles = 2 * 7 * 24 * 12 * 1.1  # Allow for slight overfetch
    logger.info(
        f"Expected candle count between {expected_min_candles:.0f} and {expected_max_candles:.0f}"
    )
    assert expected_min_candles <= row_count <= expected_max_candles
    # Check first and last timestamps
    first_timestamp_dt = datetime.datetime.fromtimestamp(
        data[0].timestamp / 1000, tz=datetime.timezone.utc
    )
    last_timestamp_dt = datetime.datetime.fromtimestamp(
        data[-1].timestamp / 1000, tz=datetime.timezone.utc
    )
    ...
    assert (
        start_dt - datetime.timedelta(minutes=10)
        <= first_timestamp_dt
        <= start_dt + datetime.timedelta(minutes=10)
    )
    assert (
