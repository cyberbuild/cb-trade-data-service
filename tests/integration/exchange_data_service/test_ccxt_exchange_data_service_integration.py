...
async def test_get_data_integration(ccxt_exchange_data_service, setup_historical_data):
    ...
    result = await ccxt_exchange_data_service.get_ohlcv_data(
...
async def test_get_data_with_enum_interval(
    ccxt_exchange_data_service, setup_historical_data
):
    ...
    logger.info(
        f"Testing _get_data_with_enum for {symbol} with interval {interval_enum.value}"
    )

    result = await ccxt_exchange_data_service.get_ohlcv_data(
        symbol, interval_enum, start_time, end_time
    )
...
async def test_sync_with_exchange_async_method(ccxt_exchange_data_service):
    ...
    logger.info(
        f"Testing sync_with_exchange for {symbol} with interval {interval.value}"
    )
...
async def test_end_to_end_sync_and_retrieve(ccxt_exchange_data_service):
    ...
    assert result is not None
    logger.info(
        f"End-to-end test completed. Retrieved {len(result.data) if result.data else 0} records"
    )
...
async def test_multiple_intervals(ccxt_exchange_data_service):
    ...
    for interval in intervals:
        ...
        start_time = end_time - timedelta(
            minutes=10
        )  # Fixed 10 minutes instead of interval-based

        data_result = await ccxt_exchange_data_service.get_ohlcv_data(
            symbol, interval, start_time, end_time
        )
