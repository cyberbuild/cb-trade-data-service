...
def test_integration_get_latest_entry_none(fetcher):
    """Test get_latest_entry returns None when no data exists"""
    result = fetcher.get_latest_entry("BTC", "binance")
    assert result is None

@pytest.mark.integration
def test_integration_get_latest_entry_with_data(fetcher, local_storage):
    ...
    local_storage.save_entry("binance", "BTC", earlier, data1)
    local_storage.save_entry("binance", "BTC", now, data2)
    result = fetcher.get_latest_entry("BTC", "binance")
...
@pytest.mark.integration
def test_integration_get_latest_entry_other_coin(fetcher, local_storage):
    ...
    local_storage.save_entry("binance", "ETH", now, data)
    result = fetcher.get_latest_entry("BTC", "binance")
    assert result is None
