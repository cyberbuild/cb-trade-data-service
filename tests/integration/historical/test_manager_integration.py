...
async def test_get_historical_data_with_paging_offset(storage_manager, setup_historical_data):
    ...
    result_first = await mgr.get_historical_data(metadata, start_time, end_time, paging_first)
    
    # Get second page
    paging_second = Paging.create(limit=5, offset=5)
    result_second = await mgr.get_historical_data(metadata, start_time, end_time, paging_second)
