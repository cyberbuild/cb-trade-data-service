...
def test_stream_historical_data_integration(local_storage):
    # Store 3 entries
    now = datetime.datetime.now(datetime.timezone.utc)
    for i in range(3):
        ts = now + datetime.timedelta(minutes=i)
        data = {"timestamp": int(ts.timestamp() * 1000), "open": 100 + i}
        local_storage.save_entry("binance", "BTC", ts, data)
    ws = DummyWS()
    mgr = HistoricalDataManagerImpl(local_storage)
    mgr.stream_historical_data("BTC", "binance", now, now + datetime.timedelta(minutes=2), ws, chunk_size=2)
...
@pytest.mark.integration
def test_stream_historical_data_websocket_error_integration(local_storage):
    now = datetime.datetime.now(datetime.timezone.utc)
    for i in range(2):
        ts = now + datetime.timedelta(minutes=i)
        data = {"timestamp": int(ts.timestamp() * 1000), "open": 100 + i}
    ...
    mgr = HistoricalDataManagerImpl(local_storage)
    mgr.stream_historical_data("BTC", "binance", now, now + datetime.timedelta(minutes=1), ws, chunk_size=1)
