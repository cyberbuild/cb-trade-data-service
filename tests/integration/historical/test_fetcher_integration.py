...
async def test_integration_fetch_data_with_stored_data(fetcher, setup_historical_data):
    ...
    min_timestamp = (
        min(item.timestamp for item in result_data.data) / 1000
    )  # Convert ms to seconds
    max_timestamp = (
        max(item.timestamp for item in result_data.data) / 1000
    )  # Convert ms to seconds
...
async def test_integration_fetch_data_with_limit_offset(fetcher, setup_historical_data):
    ...
    if len(sorted_data) < 3:
        pytest.skip(
            "Not enough data fetched by setup fixture to test limit/offset effectively."
        )
...
async def test_integration_fetch_data_time_range(
    fetcher, setup_historical_data, env_logger_func
):  # Add env_logger_func fixture
    env_logger_func(
        logger,
        context_message=f"Inside test_integration_fetch_data_time_range ({setup_historical_data.get('exchange')}) before any operations",
    )  # Call via fixture
    ...
    result_data = await fetcher.fetch_data(
        metadata=metadata, start_time=fetch_start_time, end_time=fetch_end_time
    )
...
async def test_integration_fetch_data_with_format_conversion(
    fetcher, setup_historical_data
):
