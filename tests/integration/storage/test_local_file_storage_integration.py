...
async def local_storage():
    ...
    # Create proper StorageConfig for LocalFileStorage
    from config import StorageConfig
    config = StorageConfig(
        type='local',
        local_root_path=str(TEST_ROOT_DIR)
    )
...
@pytest.mark.asyncio
async def test_save_and_get_latest_entry(local_storage: LocalFileStorage):
    """Test saving an entry and retrieving the latest one."""
    ts1 = datetime.now(timezone.utc)
    data1 = {"price": 50000, "volume": 10}
    await local_storage.save_entry(TEST_EXCHANGE, TEST_COIN, ts1, data1)
    ...
    await local_storage.save_entry(TEST_EXCHANGE, TEST_COIN, ts2, data2)

    latest_entry = await local_storage.get_latest_entry(TEST_EXCHANGE, TEST_COIN)
...
@pytest.mark.asyncio
async def test_get_range(local_storage: LocalFileStorage):
    ...
    for ts, data in zip(timestamps, entries):
        await local_storage.save_entry(TEST_EXCHANGE, TEST_COIN, ts, data)
    ...
    retrieved_entries = await local_storage.get_range(TEST_EXCHANGE, TEST_COIN, start_time_query, end_time_query)
...
@pytest.mark.asyncio
async def test_list_coins(local_storage: LocalFileStorage):
    ...
    for coin in coins:
        await local_storage.save_entry(TEST_EXCHANGE, coin, ts, data)

    listed_coins = await local_storage.list_coins(TEST_EXCHANGE)
    assert sorted(listed_coins) == sorted(coins)

@pytest.mark.asyncio
async def test_check_coin_exists(local_storage: LocalFileStorage):
    """Test checking if a coin directory exists."""
    ts = datetime.now(timezone.utc)
    data = {"price": 100}

    assert not await local_storage.check_coin_exists(TEST_EXCHANGE, TEST_COIN)
    await local_storage.save_entry(TEST_EXCHANGE, TEST_COIN, ts, data)
    assert await local_storage.check_coin_exists(TEST_EXCHANGE, TEST_COIN)

@pytest.mark.asyncio
async def test_get_latest_entry_no_data(local_storage: LocalFileStorage):
    """Test getting the latest entry when no data exists."""
    latest_entry = await local_storage.get_latest_entry(TEST_EXCHANGE, "NONEXISTENT")
    assert latest_entry is None

@pytest.mark.asyncio
async def test_get_range_no_data(local_storage: LocalFileStorage):
    """Test getting a range when no data exists."""
    start_time = datetime.now(timezone.utc) - timedelta(days=1)
    end_time = datetime.now(timezone.utc)
    retrieved_entries = await local_storage.get_range(TEST_EXCHANGE, "NONEXISTENT", start_time, end_time)
    assert len(retrieved_entries) == 0

@pytest.mark.asyncio
async def test_list_coins_no_exchange(local_storage: LocalFileStorage):
    """Test listing coins when the exchange directory doesn't exist."""
    listed_coins = await local_storage.list_coins("NONEXISTENT_EXCHANGE")
    assert len(listed_coins) == 0

@pytest.mark.asyncio
async def test_check_coin_exists_no_exchange(local_storage: LocalFileStorage):
    """Test checking coin existence when the exchange directory doesn't exist."""
    assert not await local_storage.check_coin_exists("NONEXISTENT_EXCHANGE", TEST_COIN)
