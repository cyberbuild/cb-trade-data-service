...
def sample_data() -> pa.Table:
    ...
    end_dt = datetime(2024, 1, 1, 9, 0, 0, tzinfo=timezone.utc)  # 10 hours of data (0-9)
...
async def test_delta_write_read(
    ...
    logger.info(f'[{backend_type}] Expected record count: {expected_count}, Actual record count: {actual_count}')
    assert actual_count == expected_count, f'[{backend_type}] Row count mismatch after read: expected {expected_count}, got {actual_count}'
    ...
    assert set(read_table.column_names) == set(sample_data.column_names), f'[{backend_type}] Column names mismatch after read'
    ...
    read_df = read_table.to_pandas().sort_values(by=timestamp_col).reset_index(drop=True)
    sample_df = sample_data.to_pandas().sort_values(by=timestamp_col).reset_index(drop=True)
    ...
    if pd.api.types.is_datetime64_ns_dtype(read_df[timestamp_col]):
        ...
        if read_df[timestamp_col].dt.tz is not None and sample_df[timestamp_col].dt.tz is None:
            sample_df[timestamp_col] = sample_df[timestamp_col].dt.tz_localize('UTC')
        elif read_df[timestamp_col].dt.tz is None and sample_df[timestamp_col].dt.tz is not None:
