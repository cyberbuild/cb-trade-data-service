...
async def _run_storage_manager_test_flow(storage_manager: IStorageManager, test_context: Dict[str, Any]):
    ...
    try:
        ...
        interval = '1h'
        candles_day1 = await ccxt_client.fetch_ohlcv_data(coin_symbol, day1_start, day1_end, interval=interval)
        candles_day2 = await ccxt_client.fetch_ohlcv_data(coin_symbol, day2_start, day2_end, interval=interval)
        candles_day3 = await ccxt_client.fetch_ohlcv_data(coin_symbol, day3_start, day3_end, interval=interval)
        ...
        for day_start, day_end in [
            ...
            all_candles.append((await ccxt_client.fetch_ohlcv_data(coin_symbol, day_start, day_end, interval=interval)).to_arrow().to_pandas())
        ...
        records = [convert_row(row) for row in initial_data_with_gaps.to_dict(orient='records')]
        ...
        print(f"Retrieved {len(retrieved_data.data) if retrieved_data else 0} rows across full range.")
        ...
        print(f"Expected record count: {expected_count}, Actual record count: {actual_count}")
        print("Expected timestamps:")
        print(sorted([int(pd.Timestamp(ts).timestamp() * 1000) if isinstance(ts, datetime) else ts for ts in initial_data_with_gaps['timestamp']]))
        print("Actual timestamps:")
        print(sorted([int(pd.Timestamp(ts).timestamp() * 1000) if isinstance(ts, datetime) else ts for ts in pd.DataFrame([r for r in retrieved_data.data])['timestamp']]))
        assert actual_count == expected_count, f"Mismatch in retrieved data count after initial save: expected {expected_count}, got {actual_count}"
        ...
        print([pd.to_datetime(ts, unit='ms', utc=True) if isinstance(ts, (int, float)) else ts for ts in df[timestamp_col]])
        gaps = find_time_gaps(df, expected_freq=expected_freq_delta, timestamp_col=timestamp_col)
        print(f"Found gaps: {gaps}")

        # Assert the specific gaps we introduced are found
        expected_gap_times = [pd.Timestamp(t).tz_convert('UTC') if isinstance(t, datetime) else t for t in gap_timestamps]
        found_gap_times = set(pd.Timestamp(g[0]).tz_convert('UTC') if isinstance(g[0], datetime) else g[0] for g in gaps)
        ...
        for t in expected_gap_times:
            ...
            assert any(abs((t_norm - fg).total_seconds()) < 1 for fg in found_gap_times), f"Expected gap at {t_norm} not detected. Found: {gaps}"
        ...
        if not gap_records.empty:
            ...
            gap_records_list = [convert_row(row) for row in gap_records.to_dict(orient='records')]
        ...
        print(f"Retrieved {len(retrieved_data_after_fill.data)} rows after filling gaps.")

        # Normalize timestamp column to pandas.Timestamp (UTC) if needed
        df_after_fill = pd.DataFrame([r for r in retrieved_data_after_fill.data])
        if not df_after_fill.empty and pd.api.types.is_integer_dtype(df_after_fill[timestamp_col]):
            df_after_fill[timestamp_col] = pd.to_datetime(df_after_fill[timestamp_col], unit='ms', utc=True)
        elif not df_after_fill.empty and pd.api.types.is_float_dtype(df_after_fill[timestamp_col]):
            df_after_fill[timestamp_col] = pd.to_datetime(df_after_fill[timestamp_col], unit='ms', utc=True)
        ...
        assert len(retrieved_data_after_fill.data) == len(expected_full_index), "Data count mismatch after filling gaps"

        # Verify no more gaps exist
        gaps_after_fill = find_time_gaps(df_after_fill, expected_freq=expected_freq_delta, timestamp_col=timestamp_col)
        ...
        for ts in expected_full_index:
            assert ts in df_after_fill[timestamp_col].tolist(), f"Expected timestamp {ts} not found after fill"
        ...
        append_records = [convert_row(row) for row in append_data.to_dict(orient='records')]
        ...
        df_day3 = pd.DataFrame([r for r in retrieved_day3_data.data]) if retrieved_day3_data else pd.DataFrame()
        if not df_day3.empty and pd.api.types.is_integer_dtype(df_day3[timestamp_col]):
            df_day3[timestamp_col] = pd.to_datetime(df_day3[timestamp_col], unit='ms', utc=True)
        ...
        assert appended_ts in df_day3[timestamp_col].tolist(), "Appended timestamp not found in Day 3 data"
        ...
        for ts in expected_day3_index:
            assert ts in df_day3[timestamp_col].tolist(), f"Expected timestamp {ts} not found in Day 3 data after append"
        ...
        df_day1 = pd.DataFrame([r for r in retrieved_day1_data.data]) if retrieved_day1_data else pd.DataFrame()
        if not df_day1.empty and pd.api.types.is_integer_dtype(df_day1[timestamp_col]):
            df_day1[timestamp_col] = pd.to_datetime(df_day1[timestamp_col], unit='ms', utc=True)
        ...
        assert df_day1[timestamp_col].max() <= day1_end + timedelta(minutes=freq_minutes)
