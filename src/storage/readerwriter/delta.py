...
from storage.backends.istorage_backend import (
    IStorageBackend,
...
class DeltaReaderWriter(IStorageWriter):
    ...
    async def load_range(
        ...
        try:
            ...
            logger.error(
                f"Error initializing DeltaTable for {table_uri} with options {storage_options}: {e}",
                exc_info=True,
            )
        ...
        if not timestamp_col or timestamp_col not in dt.schema().to_pyarrow().names:
            ...
            found = next(
                (
                    ...
                    if pa.types.is_timestamp(pyarrow_schema.field(name).type)
                ),
                None,
            )
        ...
        if timestamp_col in pyarrow_schema.names:
            ...
            logger.warning(
                f"Timestamp column '{timestamp_col}' not found in Delta table schema. Cannot apply time filter."
            )
        ...
        logger.info(
            f"Loaded {arrow_table.num_rows} rows from Delta table {table_uri} before limit/offset."
        )
    ...
    async def save_data(
        ...
        logger.debug(
            f"Saving data to Delta table: {table_uri}, mode: {mode}, partitions: {partition_cols}, options: {resolved_storage_options}"
        )
        ...
        if partition_cols and all(
            col in ["year", "month", "day"] for col in partition_cols
        ):
            if "timestamp" in data.schema.names:
                try:
                    ...
                    logger.error(
                        f"Error processing timestamp for partitioning columns: {e}",
                        exc_info=True,
                    )
            else:
                logger.warning(
                    "Partitioning by year/month/day requested, but 'timestamp' column not found in data."
                )
        ...
        try:
            ...
            logger.info(
                f"Successfully wrote {data.num_rows} rows to Delta table: {table_uri}"
            )
        except Exception as e:
            logger.error(
                f"Error writing Delta table {table_uri} with options {storage_options}: {e}",
                exc_info=True,
            )
            raise  # Re-raise the exception after logging
    ...
    async def save_table(
        ...
        await self.save_data(self.backend, path, data_table, {}, mode, partition_cols)
