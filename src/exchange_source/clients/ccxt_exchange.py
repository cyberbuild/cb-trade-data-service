...
class CCXTExchangeClient(IExchangeAPIClient):
    ...
    async def close(self):
        ...
        try:
            ...
            logger.error(
                f"Error closing connection for exchange {self.exchange_id}: {e}"
            )
    ...
    async def check_coin_availability(self, coin_symbol: str) -> bool:
        try:
            ...
            logger.error(
                f"Network error checking coin availability for {coin_symbol} on {self.exchange_id}: {e}"
            )
            return False
        except ccxt.ExchangeError as e:
            logger.error(
                f"Exchange error checking coin availability for {coin_symbol} on {self.exchange_id}: {e}"
            )
            return False
        except Exception as e:
            logger.error(
                f"Unexpected error checking coin availability for {coin_symbol} on {self.exchange_id}: {e}"
            )
            return False
    ...
    async def fetch_ohlcv_data(
        ...
        try:
            ...
            if coin_symbol not in self._exchange.markets:
                logger.error(
                    f"Symbol {coin_symbol} not available on {self.exchange_id}"
                )
                raise ValueError(
                    f"Symbol {coin_symbol} not available on {self.exchange_id}"
                )
            ...
            while since < end_time:
                logger.info(
                    f"Fetching data for {coin_symbol} from {datetime.datetime.fromtimestamp(since/1000, tz=datetime.timezone.utc)} (ts: {since}) on {self.exchange_id}"
                )
                try:
                    candles = await self._exchange.fetch_ohlcv(
                        symbol=coin_symbol, timeframe=interval, since=since, limit=limit
                    )
                except ccxt.RateLimitExceeded as e:
                    logger.warning(
                        f"Rate limit exceeded on {self.exchange_id}. Retrying after delay... Error: {e}"
                    )
                    ...
                    logger.error(
                        f"Network/Exchange error fetching {coin_symbol} from {self.exchange_id}: {e}. Stopping fetch for this range."
                    )
                    break
                except ccxt.ExchangeError as e:
                    logger.error(
                        f"Exchange specific error fetching {coin_symbol} from {self.exchange_id}: {e}. Stopping fetch for this range."
                    )
                    break

                if not candles:
                    logger.info(
                        f"No more candles returned for {coin_symbol} starting from {since} on {self.exchange_id}."
                    )
                    break
                ...
                if prev_since == last_timestamp or last_timestamp >= end_time:
                    logger.info(
                        f"Stopping fetch for {coin_symbol}: No new data or end_time reached."
                    )
                    break
            ...
            logger.exception(
                f"Unexpected error fetching historical data for {coin_symbol} on {self.exchange_id}: {e}"
            )
            metadata = {
                "data_type": "ohlcv",
    ...
    def _parse_interval_ms(self, interval: str) -> int:
        ...
        if (
            hasattr(self._exchange, "timeframes")
            and interval in self._exchange.timeframes
        ):
        ...
        # Fallback to 5m
        logger.warning(
            f"Interval '{interval}' not found in exchange timeframes, falling back to 5 minutes."
        )
        return 5 * 60 * 1000
